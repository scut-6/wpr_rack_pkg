...
// 抓取偏移量参数
static float fGrabOffsetX = -0.01;
static float fGrabOffsetY = -0.01;

// 放置偏移量参数
static float fPlaceOffsetX = -0.07;  //物品放置的前后距离偏移量
static float fPlaceOffsetY = -0.03;  //物品放置的左右偏移量
static float fPlaceOffsetZ = -0.02;  //物品放置的高度偏移量

// 每个隔层的抓取和放置高度设定
static stOperateHeight grabHeight[2];
void InitGrabHeight()
{
    grabHeight[0].fMinZ = 0.7;
    grabHeight[0].fMaxZ = 1.1;
    grabHeight[0].fHeight = 0.77;

    grabHeight[1].fMinZ = 1.1;
    grabHeight[1].fMaxZ = 1.5;
    grabHeight[1].fHeight = 1.17;
}
...
float GetGrabHeight(float inObjHeight)
{
    for(int i=0;i<2;i++)
    {
        if(inObjHeight >= grabHeight[i].fMinZ && inObjHeight <= grabHeight[i].fMaxZ)
        {
            return grabHeight[i].fHeight;
        }
    }
...
void ObjectsBoxCB(const wpr_rack_pkg::ObjectBox::ConstPtr &msg)
{
    if(nStep == STEP_OBJ_DETECT)
    {
        ...
        if(nNumObj > 0)
        {
            ...
            float obj_y = msg->yMin[nGrabIndex];
            float obj_z = msg->zMin[nGrabIndex];
            ROS_WARN("[Grab] Obj_%d (%.2f , %.2f , %.2f)",nGrabIndex,obj_x,obj_y,obj_z);
            grab_msg.position.x = obj_x + fGrabOffsetX;
            grab_msg.position.y = obj_y + fGrabOffsetY;
            grab_msg.position.z = GetGrabHeight(obj_z);
...
void GrabResultCB(const std_msgs::String::ConstPtr &msg)
{
    ...
    if(nStep == STEP_OBJ_GRAB)
    {
        ...
        if( nFindIndex >= 0 )
        {
            ...
            {
                int nPlaceIndex = 0;
                place_msg.position.x = arPlacePose[nPlaceIndex].position.x + fPlaceOffsetX;
                place_msg.position.y = arPlacePose[nPlaceIndex].position.y + fPlaceOffsetY;
                place_msg.position.z = GetGrabHeight(arPlacePose[nPlaceIndex].position.z) + fPlaceOffsetZ;
                ROS_INFO("pz= %.2f | gz= %.2f | offsetz= %.2f",arPlacePose[nPlaceIndex].position.z,GetGrabHeight(arPlacePose[nPlaceIndex].position.z),fPlaceOffsetZ);
...
{
    ros::init(argc, argv, "wpr1_storage_rack");
    ROS_INFO("wpr1_storage_rack start!");
    InitGrabHeight();

    ros::NodeHandle n;
    vel_pub = n.advertise<geometry_msgs::Twist>("/cmd_vel", 10);
    rack_pub = n.advertise<std_msgs::String>("/rack/command", 10);
    grab_pub = n.advertise<geometry_msgs::Pose>("/wpr1/grab_rack", 1);
    ...
    // place_msg.position.y = 0.2;
    // place_msg.position.z = 0.8;
    // place_pub.publish(place_msg);

    // grab_msg.position.x = 1.0;
    // grab_msg.position.y = 0.15;
    // grab_msg.position.z = 0.96;
    // grab_pub.publish(grab_msg);
    // ros::spin();
